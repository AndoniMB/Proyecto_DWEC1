...
// funcion que recibe un equipo con los campos que se quieran actualizar en la db. 
// Recibe un identificador para saber cual es el equipo a actualizar
// y también recibe un objeto equipo, que tiene el siguiente aspecto (según los campos a actualizar):
// reroll (int)
export async function patchEquipo(id_equipo, equipo) {
    try {
        const response = await fetch("http://localhost:3000/equipos/" + id_equipo, {
            method: "PATCH",
            headers: {
                "Content-Type": "application/json"
            },
